...
protected:
  ...
  void Connect(const Node &node) override {
    ...
    if (my_node_.id != Node::kEmpty) {
      std::string my_id = "ps" + std::to_string(my_node_.id) + "mpi" +
                          std::to_string(my_node_.rank_mpi);
  ...
  int SendMsg(const Message &msg) override {
    ...
    if (my_node_.rank_mpi != rank_dis) {
      ...
      PS_VLOG(1) << "Send: " << my_node_.rank_mpi << "->" << rank_dis
                 << " ---- " << strlen(meta_buf_mpi) << "???" << meta_size_mpi;
      MPI_Send(&meta_size_mpi, 1, MPI_INT, rank_dis, 0, MPI_COMM_WORLD);
      MPI_Send(meta_buf_mpi, meta_size_mpi, MPI_CHAR, rank_dis, 1,
               MPI_COMM_WORLD);
      PS_VLOG(1) << "Send: " << my_node_.rank_mpi << "->" << rank_dis
                 << "  --DONE--";
      ...
      for (int i = 0; i < tmp; ++i) {
        ...
        MPI_Send(data->data(), data_size, MPI_CHAR, rank_dis, 4,
                 MPI_COMM_WORLD);
        PS_VLOG(1) << "DATA Send: " << my_node_.rank_mpi << "->" << rank_dis
                   << "  --DONE--";
      }
  ...
  int RecvMsg(Message *msg) override {
    ...
    // if (my_node_.rank_mpi == 2) PS_VLOG(1) <<
    ...
    if (my_node_.rank_mpi == receiver_mpi_) {
      ...
      PS_VLOG(1) << "Recv: " << my_node_.rank_mpi << "<-" << receiver_mpi_
                 << " ---- " << strlen(meta_buf) << "???" << meta_size;
      MPI_Recv(&meta_size, 1, MPI_INT, receiver_mpi_, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      MPI_Recv(meta_buf, meta_size + 1000, MPI_CHAR, receiver_mpi_, 1,
      ...
      int data_n = 0;
      MPI_Recv(&data_n, 1, MPI_INT, receiver_mpi_, 2, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      ...
      for (int i = 0; i < data_n; i++) {
        ...
        PS_VLOG(1) << "DATA Recv: " << my_node_.rank_mpi << "<-"
                   << receiver_mpi_;
        MPI_Recv(&data_size, 1, MPI_INT, receiver_mpi_, 3, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
        data_buf = new char[data_size * 8 + 100];
        MPI_Recv(data_buf, 200000, MPI_CHAR, receiver_mpi_, 4, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
        PS_VLOG(1) << "DATA Recv: " << my_node_.rank_mpi << "<-"
                   << receiver_mpi_ << "  --DONE--";
        ...
        // if (msg->data.size()>0) PS_VLOG(1) << "[RECV DATA]" <<
        // SArray<uint64_t>(msg->data[0]);
      }
    }
